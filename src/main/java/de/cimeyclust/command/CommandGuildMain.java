            ...
            if (args.length >= 1) {
                try {
                    ...
                    if (currentArgument.equals("create")) {
                        if (args.length == 3) {
                            if(this.plugin.getPlayerAPI().getPlayerGuildState(((Player) sender).getPlayer().getName()).equals("Einsiedler")) {
                                    ...
                                    if(this.plugin.getPlotAPI().getPlotOwner(((Player) sender).getChunk()).equals(((Player) sender).getPlayer().getName()))
                                    {
                                        try{
                                            ...
                                            if(status.equals("privat") || status.equals("publik"))
                                            {
                                                ...
                                                this.plugin.getPlayerAPI().setPlayerGuildState(((Player) sender).getPlayer());
